...
export default class Graph {
    ...
    createNodes(flightMap) {
        flightMap.Quotes.forEach((flight, idx) => {
            ...
            var originInfo = flightMap.Places.filter(res =>
                res.PlaceId === flightMap.Quotes[idx].OutboundLeg.OriginId
            );
            var destinyInfo = flightMap.Places.filter(res =>
                res.PlaceId === flightMap.Quotes[idx].OutboundLeg.DestinationId
            );
            var originAirport = originInfo.IataCode;
            var destinyAirport = destinyInfo.IataCode;
